...
class CovidRisk(object):
    def __init__(
        ...
        self.selected_nfl_season_week = datetime.strptime(
            "{0} {1} 1".format(str(self.season), str(self.week + 36)),
            "%G %V %u",
        )
        ...
        self.team_abbrev_conversion_dict = {"JAC": "JAX", "LA": "LAR"}

        self.raw_covid_data = {}
        self.raw_covid_data_file_path = os.path.join(
            data_dir, "covid_raw_data.json"
        )
        ...
        if self.refresh or not self.dev_offline:
            if not self.covid_data and self.season >= 2020:
                ...
                football_db_endpoint = "https://www.footballdb.com/transactions/index.html?period={0}&period={1}".format(
                    str(self.season + 1), str(self.season)
                )
                headers = {
                ...
                for row in soup.findAll(attrs={"class": "td w25 rowtitle"}):
                    ...
                    transactions_html = row.findNext(
                        attrs={"class": "td w75 td-clear"}
                    )

                    if "covid" in str(transactions_html.text).lower():
                        transaction_date = row.findPrevious(
                            attrs={"class": "stacktable-title"}
                        ).text
                        transaction_team = nfl_team_abbrev_ref.get(row.b.text)

                        if (
                            datetime.strptime(transaction_date, "%B %d, %Y")
                            <= self.selected_nfl_season_week
                        ):
                            for transaction in str(transactions_html).split(
                                "."
                            ):
                                if "covid" in transaction.lower():
                                    for player in BeautifulSoup(
                                        transaction, "html.parser"
                                    ).findAll("a"):
                                        transaction_action = ""
                                        if "placed" in transaction.lower():
                                            transaction_action = "add"
                                        elif (
                                            "activated" in transaction.lower()
                                        ):
                                        ...
                                        covid_transactions.append(
                                            player_transaction
                                        )
                                        self.add_entry(
                                            player.text, player_transaction
                                        )
                ...
                for team, data in self.raw_covid_data.items():
                    ...
                    data["transactions"] = {
                        ...
                        for key, group in itertools.groupby(
                            sorted(
                                data["transactions"], key=lambda x: x["action"]
                            ),
                            lambda x: x["action"],
                        )
                    }
        ...
        if len(self.covid_data) == 0 and self.season >= 2020:
            ...
            logger.warning(
                "COVID-19 was not a factor during NFL seasons prior to 2020, so metric is being ignored."
            )
        else:
            logger.info(
                "{0} players at risk of COVID-19 were loaded".format(
                    len(self.covid_data)
                )
            )
    ...
    def open_covid_data(self):
        ...
        if os.path.exists(self.covid_data_file_path):
            with open(
                self.covid_data_file_path, "r", encoding="utf-8"
            ) as covid_in:
                self.covid_data = dict(json.load(covid_in))

        if os.path.exists(self.raw_covid_data_file_path):
            with open(
                self.raw_covid_data_file_path, "r", encoding="utf-8"
            ) as covid_raw_in:
    ...
    def save_covid_data(self):
        if self.save_data:
            ...
            with open(
                self.covid_data_file_path, "w", encoding="utf-8"
            ) as covid_out:
                json.dump(
                    self.covid_data, covid_out, ensure_ascii=False, indent=2
                )

            # save raw player covid data locally
            with open(
                self.raw_covid_data_file_path, "w", encoding="utf-8"
            ) as covid_raw_out:
                json.dump(
                    self.raw_covid_data,
                    covid_raw_out,
    ...
    def add_entry(self, player_full_name, player_transaction):
        ...
        if player_transaction:
            if player_full_name not in self.covid_data:
                ...
                self.covid_data.get(player_full_name).get(
                    "transactions"
                ).append(player_transaction)

                if datetime.strptime(
                    self.covid_data.get(player_full_name).get("last_date"),
                    "%B %d, %Y",
                ) < datetime.strptime(
                    player_transaction.get("date"), "%B %d, %Y"
                ):
                    self.covid_data.get(player_full_name)[
                        "last_date"
                    ] = player_transaction.get("date")
    ...
    def get_player_covid_risk(
        self, player_full_name, player_team_abbr, player_pos
    ):
        ...
        if team_abbr in self.raw_covid_data.keys():
            ...
            for transaction in (
                self.raw_covid_data.get(team_abbr)
                .get("transactions")
                .get("add")
            ):
            ...
            covid_risk_score += (
                self.raw_covid_data.get(team_abbr).get("count") - 1
            )
            ...
            if (
                covid_recency < timedelta(days=14)
                and not player_on_covid_list_present
            ):
                ...
                covid_risk_score += (
                    (10 - recency_risk) if 10 >= recency_risk >= 0 else 0
                )
    ...
    def generate_covid_risk_json(self):
        ordered_covid_risk_data = OrderedDict(
            sorted(self.raw_covid_data.items(), key=lambda k_v: k_v[0])
        )
        with open(
            self.raw_covid_data_file_path, mode="w", encoding="utf-8"
        ) as covid_data:
            json.dump(
                ordered_covid_risk_data,
                covid_data,
                ensure_ascii=False,
                indent=2,
            )
