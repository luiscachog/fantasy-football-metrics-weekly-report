...
class CalculateMetrics(object):
        ...
        for division in sorted_divisions.values():
            ...
            for team in division:
                ...
                if league.is_faab and team.waiver_priority != 0:
                    team_division_standings_data.insert(
                        8, team.waiver_priority
                    )
    ...
    def get_median_standings_data(league: BaseLeague):
        ...
        for team in sorted(
            ...
            team_current_median_standings_data = [
                ...
                "{:.2f}".format(
                    round(
                        float(team.current_median_record.get_points_for()), 2
                    )
                ),
                team.current_median_record.get_streak_str(),
                "{:.2f}".format(
                    team.current_median_record.get_points_against()
                ),
            ]

            current_median_standings_data.append(
                team_current_median_standings_data
            )
            rank += 1
    ...
    def get_playoff_probs_data(league_standings, data_for_playoff_probs):
        ...
        sorted_playoff_probs_data = sorted(
            playoff_probs_data,
            key=lambda x: (x[-2], x[-1], x[prob_ndx]),
            reverse=True,
        )
        for team_playoff_probs_data in sorted_playoff_probs_data:
            ...
            team_playoff_probs_data[prob_ndx] = (
                "%.2f%%" % team_playoff_probs_data[prob_ndx]
            )
            if team_playoff_probs_data[prob_ndx + 1] == 1:
                team_playoff_probs_data[prob_ndx + 1] = (
                    "%d win" % team_playoff_probs_data[prob_ndx + 1]
                )
            else:
                team_playoff_probs_data[prob_ndx + 1] = (
                    "%d wins" % team_playoff_probs_data[prob_ndx + 1]
                )
            ndx = prob_ndx + 2
    ...
    def get_coaching_efficiency_data(self, coaching_efficiency_results):
        ...
        for team in coaching_efficiency_results:  # type: BaseTeam
            ...
            if ranked_coaching_efficiency == "DQ":
                ...
                ranked_coaching_efficiency = "%.2f%%" % float(
    ...
    def get_covid_risk_rank_data(covid_risk_results):
        ...
        for team in covid_risk_results:  # type: BaseTeam
            ...
            covid_risk_data.append(
                [ndx, ranked_team_name, ranked_team_manager, ranked_covid_risk]
            )
            ndx += 1
    ...
    def resolve_coaching_efficiency_ties(
        ...
        if league.player_data_by_week_function:
            ...
            if (
                ...
                and int(week) == int(week_for_report)
            ):
                for ce_result in data_for_coaching_efficiency:
                    if ce_result[0] == "1*":
                            ...
                            if player.selected_position not in bench_positions:
                                ...
                                while week_counter <= int(week):
                                    ...
                                    if (
                                        ...
                                        in players_by_week.keys()
                                    ):
                                        weekly_player_points = players_by_week[
                                            str(player.player_id)
                                        ].points
                                    else:
                                        weekly_player_points = (
                                            league.get_player_data_by_week(
                                                str(player.player_id),
                                                str(week_counter),
                                            )
                                        )
                        ...
                        coaching_efficiency_results_data_with_tiebreakers.append(
                            ce_result
                        )
                    else:
    ...
    def calculate_records(week, league: BaseLeague, custom_weekly_matchups):
        ...
        standings = (
            league.standings if league.standings else league.current_standings
        )
        ...
        for team in standings:  # type: BaseTeam
            if week == 1:
                ...
                previous_week_record = league.records_by_week[
                    str(int(week) - 1)
                ][
                    team.team_id
                ]  # type: BaseRecord
        ...
        for team_1 in teams.values():  # type: BaseTeam
            ...
            if luck_record.get_wins() != 0 and luck_record.get_losses() != 0:
                ...
                if matchup_result == "W" or matchup_result == "T":
                    ...
                    luck = (
                        0
                        - (luck_record.get_wins() + luck_record.get_ties())
                        / num_teams
                    )
    ...
    def get_ranks_for_metric(
        data_for_metric, power_ranked_teams, metric_ranking_key
    ):
        rank = 1
    ...
    def calculate_power_rankings(
        ...
        self.get_ranks_for_metric(
            data_for_scores, power_ranked_teams, "score_ranking"
        )
        self.get_ranks_for_metric(
            data_for_coaching_efficiency,
            power_ranked_teams,
        ...
        self.get_ranks_for_metric(
            data_for_luck, power_ranked_teams, "luck_ranking"
        )
    ...
    def calculate_z_scores(weekly_teams_results):
        ...
        for team_id in weekly_teams_results[0].keys():
            ...
            if can_calculate:
                scores = [
                    week[team_id].points for week in weekly_teams_results
                ]
