...
class BeefStats(object):
    def __init__(
        ...
        self.fox_sports_public_api_key = {
            "apikey": "jE7yBJVRNAwdDesMgTzTXUUSx1It41Fq"
        }
        self.teams_url = (
            "https://api.foxsports.com/sportsdata/v1/football/nfl/teams.json"
        )
        self.tabbu_value = 500.0

        self.raw_player_data = {}
        self.raw_player_data_file_path = os.path.join(
            data_dir, "beef_raw_data.json"
        )
        ...
        if self.refresh or not self.dev_offline:
            if not self.beef_data:
                ...
                fox_sports_nfl_teams_data = requests.get(
                    self.teams_url, params=self.fox_sports_public_api_key
                ).json()

                for team in fox_sports_nfl_teams_data.get("page"):
                    team_url = team.get("links").get("api").get("athletes")
                    team_roster = requests.get(
                        team_url, self.fox_sports_public_api_key
                    ).json()
        ...
        if len(self.beef_data) == 0:
            ...
            logger.info(
                "{0} player weights/TABBUs were loaded".format(
                    len(self.beef_data)
                )
            )
    ...
    def open_beef_data(self):
        ...
        if os.path.exists(self.beef_data_file_path):
            with open(
                self.beef_data_file_path, "r", encoding="utf-8"
            ) as beef_in:
    ...
    def save_beef_data(self):
        if self.save_data:
            ...
            with open(
                self.beef_data_file_path, "w", encoding="utf-8"
            ) as beef_out:
                json.dump(
                    self.beef_data, beef_out, ensure_ascii=False, indent=2
                )
    ...
    def add_entry(self, player_full_name, player_json=None, team_json=None):
        ...
        if player_json:
            ...
            player_beef_dict = {
                ...
                "tabbu": float(player_json.get("weight"))
            ...
            if player_json.get("position").get("abbreviation") in [
                "CB",
                "LB",
                "DE",
                "DT",
                ...
                if team_json.get("abbreviation") not in self.beef_data.keys():
                    ...
                    weight = self.beef_data[team_json.get("abbreviation")].get(
                        "weight"
                    ) + float(player_json.get("weight"))
                    tabbu = self.beef_data[team_json.get("abbreviation")].get(
                        "tabbu"
                    ) + (float(player_json.get("weight")) / self.tabbu_value)

                    team_def_entry = self.beef_data[
                        team_json.get("abbreviation")
                    ]
                    team_def_entry["weight"] = weight
                    team_def_entry["tabbu"] = tabbu
                    team_def_entry["players"][
                        player_full_name
                    ] = player_beef_dict
        else:
            player_beef_dict = {
    ...
    def get_player_beef_stat(
        self, player_first_name, player_last_name, player_team_abbr, key_str
    ):
    ...
    def get_player_weight(
        self, player_first_name, player_last_name, team_abbr
    ):
        return self.get_player_beef_stat(
            player_first_name, player_last_name, team_abbr, "weight"
        )

    def get_player_tabbu(self, player_first_name, player_last_name, team_abbr):
        return self.get_player_beef_stat(
            player_first_name, player_last_name, team_abbr, "tabbu"
        )

    def generate_player_info_json(self):
        ordered_player_data = OrderedDict(
            sorted(self.raw_player_data.items(), key=lambda k_v: k_v[0])
        )
        with open(
            self.raw_player_data_file_path, mode="w", encoding="utf-8"
        ) as player_data:
            json.dump(
                ordered_player_data, player_data, ensure_ascii=False, indent=2
            )
