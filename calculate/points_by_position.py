...
class PointsByPosition(object):
    def __init__(self, league: BaseLeague, week_for_report):
        ...
        self.flex_types.remove(
            "FLEX_IDP"
        )  # comment/uncomment line to remove/add FLEX_IDP to team points by position
    ...
    def calculate_points_by_position_season_averages(
        ...
        for team in list(season_average_points_by_position_dict.keys()):
            points_by_position = season_average_points_by_position_dict.get(
                team
            )
            season_average_points_by_position = {}
            for week in points_by_position:
                for position in week:
                    position_points = season_average_points_by_position.get(
                        position[0]
                    )
                    if position_points:
    ...
    def execute_points_by_position(self, team_name, roster):
        logger.debug(
            'Calculating points by position for team "{0}".'.format(team_name)
        )
        ...
        starting_players = [
            p
            for p in roster
            if p.selected_position not in self.bench_positions
        ]
        for slot in list(self.roster_slot_counts.keys()):
            if (
                slot not in self.bench_positions
                and slot not in self.flex_types
            ):
                player_points_by_position.append(
                    [
                        slot,
                        self.get_points_for_position(starting_players, slot),
                    ]
                )
    ...
    def get_weekly_points_by_position(self, teams_results):
        ...
        for team_result in teams_results.values():  # type: BaseTeam
            ...
            player_points_by_position = self.execute_points_by_position(
