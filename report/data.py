...
from utils.report_tools import (
    add_report_team_stats,
    get_player_game_time_statuses,
)
...
class ReportData(object):
    def __init__(
        ...
        inactive_players = []
        if dq_ce:
            injured_players = get_player_game_time_statuses(
                week_counter, league
            ).findAll("div", {"class": "tr"})
            for player in injured_players:
                player_name = player.find("a").text.strip()
                player_status_info = player.find(
        ...
        league.standings = sorted(
            ...
            key=lambda x: (
                ...
                -league.records_by_week[str(week_counter)][
                    x.team_id
                ].get_points_for(),
            ),
        )
        ...
        if int(week_counter) == int(week_for_report):
            disqualified_teams = config.get(
                "Settings", "coaching_efficiency_disqualified_teams"
            )
        ...
        self.data_for_current_standings = (
            metrics_calculator.get_standings_data(league)
        )
        ...
        if self.data_for_playoff_probs:
            self.data_for_playoff_probs = (
                metrics_calculator.get_playoff_probs_data(
        ...
        z_score_rank = 1
        if all(
            z_score_val is None for z_score_val in z_score_results.values()
        ):
        ...
        self.data_for_weekly_points_by_position = (
            points_by_position.get_weekly_points_by_position(
                self.teams_results
            )
        )
        ...
        self.data_for_optimal_scores = (
            metrics_calculator.get_optimal_score_data(
                sorted(
                    self.teams_results.values(),
        ...
        self.data_for_covid_risk_rankings = (
            metrics_calculator.get_covid_risk_rank_data(
                sorted(
                    self.teams_results.values(),
        ...
        self.num_first_place_for_luck = len(
            [
                list(group)
                for key, group in itertools.groupby(
                    self.data_for_luck, lambda x: x[3]
                )
            ][0]
        )
        ...
        self.data_for_bad_boy_rankings = [
            result
            for result in self.data_for_bad_boy_rankings
            if int(result[5]) != 0
        ]
        ...
        for team in self.data_for_teams:
            for team_id in power_ranking_results.keys():
                if team[0] == team_id:
                    team.append(
                        power_ranking_results[team_id]["power_ranking"]
                    )
        ...
        if self.num_coaching_efficiency_dqs > 0:
            ...
            for (
                ...
                if ineligible_players_count == -1:
                    ce_dq_str += "{0} (incomplete active squad), ".format(
                        team_name
                    )
                elif ineligible_players_count == -2:
                    ce_dq_str += "{0} (manually disqualified), ".format(
                        team_name
                    )
                else:
                    ce_dq_str += (
                        "{0} (ineligible bench players: {1}/{2}), ".format(
                            team_name,
                            ineligible_players_count,
                            league.roster_position_counts.get("BN"),
                        )
                    )  # exclude IR
            weekly_metrics_output_string += (
                "   COACHING EFFICIENCY DQs: {0}\n".format(ce_dq_str[:-2])
            )
