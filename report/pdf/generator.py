...
def get_player_image(
    ...
    headshots_dir = os.path.join(
        data_dir, "week_" + str(week), "player_headshots"
    )
    ...
    if url:
        ...
        local_img_jpg_path = os.path.join(
            headshots_dir, img_name.split(".")[0] + ".jpg"
        )
        ...
        if not os.path.exists(local_img_jpg_path):
            if not os.path.exists(local_img_path):
                if not dev_offline:
                    logger.debug(
                        'Retrieving player headshot for "{0}"'.format(
                            player_name
                        )
                    )
                    try:
                        ...
                        logger.error(
                            "Unable to retrieve player headshot{0} at url {1}".format(
                        ...
                        local_img_path = os.path.join(
                            "resources", "images", "photo-not-available.jpg"
                        )
                else:
                    logger.error(
        ...
        logger.error(
            "No available URL for player{0}.".format(
    ...
    scaled_img = ReportLabImage(
        local_img_path, width=width, height=(width * aspect)
    )
...
class HyperlinkedImage(ReportLabImage, object):
    ...
    def drawOn(self, canvas, x, y, _sW=0):
        if (
            self.hyperlink
        ):  # If a hyperlink is given, create a canvas.linkURL()
...
class PdfGenerator(object):
    def __init__(
        ...
        self.widths_n_cols_no_1 = [
            ...
            round(3.4 / self.playoff_slots, 2) * inch
        ] * self.playoff_slots  # 8.20
        ...
        supported_fonts = [
            font.strip()
            for font in self.config.get("Report", "supported_fonts").split(",")
        ]
        if font_key not in supported_fonts:
            logger.warning(
                "The {0} font is not supported at this time. Report formatting has defaulted to Helvetica. "
        ...
        if use_custom_font:
            ...
            pdfmetrics.registerFont(
                TTFont(
                    self.font_italic, "resources/fonts/" + self.font + ".ttf"
                )
            )
            pdfmetrics.registerFont(
                TTFont(
                    self.font_bold_italic,
                    "resources/fonts/" + self.font + ".ttf",
                )
            )
        ...
        self.style_left_align_right_col = TableStyle(
        ...
        self.optimal_scores_headers = [
            ["Place", "Team", "Manager", "Optimal Points", "Season Total"]
        ]
        ...
        footer_data = [
            ...
            [
                Paragraph(
                    "bc1qataspvklhewtswm357m0677q4raag5new2xt3e",
                    self.text_style_small,
                ),
                Paragraph(
                    "0x5eAa522e66a90577D49e9E72f253EC952CDB4059",
                    self.text_style_small,
                ),
                Paragraph(
                    "nano_3ug3o6yy983jsqdsc773izhr3jfz4dq8bz7yfhhzkkeq7s8ern1ws7dng4pq",
                    self.text_style_small,
                ),
            ],
        ]
        ...
        self.data_for_median_standings = (
            report_data.data_for_current_median_standings
        )
        self.data_for_scores = report_data.data_for_scores
        self.data_for_coaching_efficiency = (
            report_data.data_for_coaching_efficiency
        )
        ...
        self.data_for_covid_risk_rankings = (
            report_data.data_for_covid_risk_rankings
        )
        ...
        self.style_tied_power_rankings = self.set_tied_values_style(
            self.report_data.ties_for_power_rankings,
            table_style_list,
            "power_ranking",
        )
        ...
        self.toc = TableOfContents(
            self.font, self.font_size, self.config, self.break_ties
        )
    ...
    def set_tied_values_style(self, num_ties, table_style_list, metric_type):
        ...
        if metric_type == "scores":
            ...
            if (
                not self.report_data.num_first_place_for_coaching_efficiency
                > 0
            ):
                num_first_places = 0
            else:
            ...
            if not self.report_data.ties_for_first_for_power_rankings > 0:
                ...
                num_first_places = (
                    self.report_data.ties_for_first_for_power_rankings
                )
            ...
            if not self.report_data.num_first_place_for_bad_boy_rankings > 0:
                ...
                num_first_places = (
                    self.report_data.num_first_place_for_bad_boy_rankings
                )
            ...
            if not self.report_data.num_first_place_for_beef_rankings > 0:
                ...
                num_first_places = (
                    self.report_data.num_first_place_for_beef_rankings
                )
        ...
        if metric_type == "coaching_efficiency":
            if self.num_coaching_efficiency_dqs > 0:
                ...
                while eff_dq_count > 0:
                    ...
                    eff_dq_count -= 1
                    dq_index += 1
                self.style_efficiency_dqs = TableStyle(
                    efficiencies_dq_table_style_list
                )
    ...
    def create_section(
        ...
        tied_metric=False,
        metric_type=None,
        section_title_function=None,
    ):
        logger.debug(
            'Creating report section: "{0}" with data:\n{1}\n'.format(
                title_text if title_text else metric_type,
        ...
        if title_text:
            ...
            self.appendix.add_entry(
                ...
                getattr(
                    descriptions,
                    title_text.replace(" ", "_").replace("-", "_").lower(),
                ),
            )
        ...
        if metric_type == "standings":
            ...
            table_style.add(
                ...
                (0, 0),
                (-1, -1),
                (self.font_size - 2) - font_reduction,
            )
        ...
        if metric_type == "beef":
            cow_icon = self.get_img(
                os.path.join("resources", "images", "cow.png"),
                width=0.20 * inch,
            )
            beef_icon = self.get_img(
                os.path.join("resources", "images", "beef.png"),
                width=0.20 * inch,
            )
            half_beef_icon = self.get_img(
                os.path.join("resources", "images", "beef-half.png"),
                width=0.10 * inch,
            )
            ...
            for team in data:
                ...
                tabbu_column_table.setStyle(
                    TableStyle(tabbu_column_table_style_list)
                )
    ...
    def get_tied_metric_footer(self, metric_type):
        ...
        if metric_type in ["scores", "coaching_efficiency"]:
            if not self.break_ties:
                return Paragraph(
                    self.tie_for_first_footer, self.text_style_normal
                )
            else:
                return None
        else:
    ...
    def create_title(
        ...
        if subtitle_text:
            ...
            subtitle = Paragraph(
                """<para align=center>""" + subtitle_text_str + """</para>""",
                self.text_style_subtitles,
            )
    ...
    def create_anchored_title(
        self, title_text, title_width=8.5, element_type=None, anchor=""
    ):
    ...
    def create_line_chart(
        ...
        # fit y-axis of table
        values = [
            weeks[1] if weeks[1] != "DQ" else 0.0
            for teams in data
            for weeks in teams
        ]
    ...
    def create_team_stats_pages(
        self,
        doc_elements,
        weekly_team_data_by_position,
        season_average_team_data_by_position,
    ):
        ...
        for team in alphabetical_teams:
            ...
            if (
                self.config.getboolean(
                    "Report", "team_points_by_position_charts"
                )
                or self.config.getboolean("Report", "team_bad_boy_stats")
                or self.config.getboolean("Report", "team_beef_stats")
                or self.config.getboolean("Report", "team_boom_or_bust")
            ):
                title = self.create_title(
                    "<i>" + team_result.name + "</i>",
                    element_type="section",
            ...
            if self.config.getboolean(
                ...
                season_data = [
                    x[1]
                    for x in season_average_team_data_by_position.get(team_id)
                ]
                ...
                team_table = Table(
                    [
                        [
                            ...
                            self.create_title(
                                "Season Average Points by Position",
                                title_width=2.00,
                            ),
                        ],
                        [
                            BreakdownPieDrawing(
                                labels, weekly_data, font=self.font
                            ),
                            BreakdownPieDrawing(
                                labels, season_data, font=self.font
                            ),
                        ],
                    ],
            ...
            if self.config.getboolean(
                "Report", "league_bad_boy_rankings"
            ...
            if self.config.getboolean(
                ...
                if player_info:
                    ...
                    beefy_players = sorted(
                        player_info, key=lambda x: x.tabbu, reverse=True
                    )
                    beefy_players_data = []
                    num_beefy_bois = 3
                    ndx = 0
                    ...
                    while count < num_beefy_bois:
                        ...
                        if player.last_name:
                            beefy_players_data.append(
                                [player.full_name, player.tabbu, player.weight]
                            )
                            count += 1
                        ndx += 1
            ...
            if self.config.getboolean("Report", "team_boom_or_bust"):
                if player_info:
                    ...
                    if (
                        any(
                            player.season_points for player in starting_players
                        )
                    ...
                    best_player_headshot = get_player_image(
                        ...
                        self.config.getint("Report", "image_quality"),
                        1.5 * inch,
                        ...
                        data.append(
                            [
                                "{0} ({1} avg: +{2}%)".format(
                                    ...
                                    if best_weekly_player.season_average_points
                                    > 0
                                    else "∞"
                                    if best_weekly_player.season_average_points
                                    == 0
                                ...
                                "{0} ({1} avg: -{2}%)".format(
                                    ...
                                    if worst_weekly_player.season_average_points
                                    > 0
                                    else "∞"
                                    if worst_weekly_player.season_average_points
                                    == 0
    ...
    def generate_pdf(self, filename_with_path, line_chart_data_list):
        ...
        if self.config.getboolean("Report", "league_standings"):
            ...
            if self.report_data.has_divisions:
                ...
                for (
                    division
                ) in self.report_data.data_for_current_division_standings:
                    ...
                    table_header = (
                        ...
                        else "Division {0}".format(division_count)
                    )
        ...
        if self.config.getboolean("Report", "league_median_standings"):
            ...
            median_standings_style.add(
                "FONT", (3, 1), (3, -1), self.font_italic
            )
            median_standings_style.add(
                "FONTSIZE", (0, 0), (-1, -1), self.font_size - 4
            )
            ...
            elements.append(
                self.create_section(
                    ...
                    subsubtitle_text="WEEK {0} LEAGUE MEDIAN SCORE: {1}".format(
                        self.week_for_report,
                        self.data_for_median_standings[0][-1],
                    ),
                )
            )
        ...
        if self.config.getboolean(
            "Report", "league_power_rankings"
        ) or self.config.getboolean("Report", "league_z_score_rankings"):
            elements.append(self.add_page_break())
        ...
        if self.config.getboolean(
            "Report", "league_coaching_efficiency_rankings"
        ):
            # coaching efficiency
            elements.append(
                self.create_section(
                    ...
                    self.widths_05_cols_no_1,
                    tied_metric=(
                        (self.report_data.ties_for_coaching_efficiency > 0)
                        and (
                            self.report_data.league.player_data_by_week_function
                            is not None
                        )
                    ),
        ...
        if (
            ...
            or self.config.getboolean("Report", "league_luck_rankings")
        ):
            elements.append(self.add_page_break())
        ...
        if self.config.getboolean(
            "Report", "league_bad_boy_rankings"
        ) or self.config.getboolean("Report", "league_beef_rankings"):
            elements.append(self.add_page_break())
        ...
        if self.config.getboolean(
            "Report", "league_weekly_top_scorers"
        ) or self.config.getboolean("Report", "league_weekly_highest_ce"):
            elements.append(self.add_page_break())

        if (
            self.config.getboolean("Report", "league_covid_risk_rankings")
            and int(self.season) >= 2020
        ):
            # covid risk rankings
            ...
            self.appendix.add_entry(
                ...
                getattr(
                    descriptions,
                    charts_covid_page_title_str.replace(" ", "_")
                    .replace("-", "_")
                    .lower(),
                ),
            )
            ...
            elements.append(
                KeepTogether(
                    self.create_3d_horizontal_bar_chart(
                        self.data_for_covid_risk_rankings, "Risk Factor", 5
                    )
                )
            )
            elements.append(self.spacer_twentieth_inch)
        ...
        if self.config.getboolean("Report", "report_team_stats") and (
            self.config.getboolean("Report", "team_points_by_position_charts")
            or self.config.getboolean("Report", "team_bad_boy_stats")
            or self.config.getboolean("Report", "team_beef_stats")
            or self.config.getboolean("Report", "team_boom_or_bust")
        ):
        ...
        # build pdf
        logger.info(
            "generating PDF ({0})...".format(filename_with_path.split("/")[-1])
        )
...
class TableOfContents(object):
    def __init__(self, font, font_size, config, break_ties):
        ...
        self.toc_style_title_right = ParagraphStyle(
            name="tocr", alignment=TA_RIGHT, fontSize=font_size, fontName=font
        )
        self.toc_style_title_left = ParagraphStyle(
            name="tocl", alignment=TA_LEFT, fontSize=font_size, fontName=font
        )
        ...
        if (
            self.config.getboolean("Report", "league_standings")
            or self.config.getboolean("Report", "league_playoff_probs")
            or self.config.getboolean("Report", "league_median_standings")
            or self.config.getboolean("Report", "league_power_rankings")
            or self.config.getboolean("Report", "league_z_score_rankings")
            or self.config.getboolean("Report", "league_score_rankings")
            or self.config.getboolean(
                "Report", "league_coaching_efficiency_rankings"
            )
            or self.config.getboolean("Report", "league_luck_rankings")
            or self.config.getboolean(
                "Report", "league_optimal_score_rankings"
            )
            or self.config.getboolean("Report", "league_bad_boy_rankings")
            or self.config.getboolean("Report", "league_beef_rankings")
        ):
            self.toc_metric_section_data = [
                [
                    Paragraph(
                        "<b><i>Metrics</i></b>", self.toc_style_title_right
                    ),
                    "",
            ...
            self.toc_appendix_data = [
                [
                    Paragraph(
                        "<b><i>Appendices</i></b>", self.toc_style_title_right
                    ),
                    "",
                    Paragraph("<b><i>Page</i></b>", self.toc_style_title_left),
                ]
        ...
        if self.config.getboolean(
            ...
            self.toc_top_performers_section_data = [
                [
                    Paragraph(
                        "<b><i>Top Performers</i></b>",
                        self.toc_style_title_right,
                    ),
                    "",
                    Paragraph("<b><i>Page</i></b>", self.toc_style_title_left),
                ]
        ...
        if self.config.getboolean(
            "Report", "report_time_series_charts"
        ) or self.config.getboolean("Report", "league_covid_risk_rankings"):
            self.toc_chart_section_data = [
                [
                    Paragraph(
                        "<b><i>Charts</i></b>", self.toc_style_title_right
                    ),
                    "",
                    Paragraph("<b><i>Page</i></b>", self.toc_style_title_left),
                ]
        ...
        if self.config.getboolean("Report", "report_team_stats") and (
            ...
            self.toc_team_section_data = [
                [
                    Paragraph(
                        "<b><i>Teams</i></b>", self.toc_style_title_right
                    ),
                    "",
                    Paragraph("<b><i>Page</i></b>", self.toc_style_title_left),
                ]
    ...
    def get_toc(self):
        return Table(
            ...
            rowHeights=[0.25 * inch] * len(self.toc_metric_section_data)
            + [0.05 * inch]
            + [0.25 * inch] * len(self.toc_top_performers_section_data)
            + [0.05 * inch]
            + [0.25 * inch] * len(self.toc_chart_section_data)
            + [0.05 * inch]
            + [0.25 * inch] * len(self.toc_team_section_data)
            + [0.05 * inch]
...
class Appendix(object):
    def __init__(
        self, title, title_formatter, toc_anchor_getter, font_size, style
    ):
        self.title = title
