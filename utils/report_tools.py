    ...
    if config_file_path.is_file():
        ...
        if create_config == "y":
            return create_config_from_template(
                config, root_directory, config_file_path
            )
        if create_config == "n":
            logger.error(
...
def create_config_from_template(
    ...
    if not platform:
        ...
        platform = input(
            "{0}For which fantasy football platform are you generating a report? ({1}) -> {2}".format(
                Fore.GREEN, "/".join(supported_platforms), Style.RESET_ALL
            )
        )
        if platform not in supported_platforms:
            logger.warning(
                "Please only select one of the following platforms: {0}".format(
                    ", or ".join(
                        [
        ...
        logger.debug(
            'Retrieved fantasy football platform for "config.ini": {0}'.format(
                platform
            )
        )
    ...
    if not league_id:
        league_id = input(
            "{0}What is your league ID? -> {1}".format(
                Fore.GREEN, Style.RESET_ALL
            )
        )
        logger.debug(
            'Retrieved fantasy football league ID for "config.ini": {0}'.format(
                league_id
            )
        )
    ...
    if not season:
        season = input(
            "{0}For which NFL season (starting year of season) are you generating reports? -> {1}".format(
                Fore.GREEN, Style.RESET_ALL
            )
        )
        try:
    ...
    if not current_week:
        ...
        try:
            if int(current_week) < 0 or int(current_week) > 17:
                logger.warning(
                    "Week {0} is not a valid NFL week. Please select a week from 1 to 17.".format(
                        current_week
                    )
                )
                time.sleep(0.25)
...
def check_for_updates(auto_run=False):
    ...
    if not active_network_connection():
        logger.info(
            "No active network connection found. Unable to check for updates for the Fantasy Football Metrics "
            "Weekly Report app."
        )
    else:
        version_tags = sorted(
        ...
        if "https" not in origin_url:
            origin_url = "https://github.com/{0}".format(
                str(project_repo.remotes.origin.url).split(":")[1]
            )
        ...
        if active_branch != target_branch:
            if not auto_run:
                switch_branch = input(
                    '{0}You are {2}not{0} on the deployment branch ({1}"{4}"{0}) of the Fantasy '
                ...
                if switch_branch == "y":
                    ...
                    logger.warning(
                        'Running the app on a branch that is not "{0}" could result in unexpected and '
                ...
                logger.info(
                    'Auto-run is set to "true". Automatically switching to deployment branch "main".'
                )
                project_repo.git.checkout(target_branch)

        num_commits_behind = len(
            list(
                project_repo.iter_commits(
                    "{0}..origin/{0}".format(target_branch)
                )
            )
        )
        ...
        if num_commits_behind > 0:
            ...
            confirm_update = input(
                up_to_date_status_msg
                + " {1}Do you wish to update the app? {0}({1}y{0}/{2}n{0}) -> {3}".format(
                    Fore.YELLOW, Fore.GREEN, Fore.RED, Style.RESET_ALL
                )
            )

            not_up_to_date_status_message = (
                "Running {0} of app. Please update to {1} for the latest features, "
...
def update_app(repository: Repo):
    ...
    if len(diff) > 0:
        logger.error(
            "There are changes to local files that could cause conflicts when updating the app "
            "automatically."
        )
        logger.warning(
            "Please update the app manually by running {0}git pull origin main{1} and resolve any "
...
def league_data_factory(
    ...
    if platform in supported_platforms:
        ...
        logger.error(
            ...
            "Please change your settings in config.ini and try again.".format(
                platform
            )
        )
        sys.exit("...run aborted.")
...
def add_report_player_stats(
    ...
    if player.selected_position not in bench_positions:
        ...
        if config.getboolean("Report", "league_beef_rankings"):
            ...
            player.weight = beef_stats.get_player_weight(
                player.first_name, player.last_name, player.nfl_team_abbr
            )
            player.tabbu = beef_stats.get_player_tabbu(
                player.first_name, player.last_name, player.nfl_team_abbr
            )

        if (
            config.getboolean("Report", "league_covid_risk_rankings")
            and int(season) >= 2020
        ):
...
def add_report_team_stats(
    ...
    for player in team.roster:
        add_report_player_stats(
            config, season, metrics, player, bench_positions
        )
    ...
    if round(team.points, 2) != (
        starting_lineup_points + team.home_field_advantage
    ):
        logger.warning(
    ...
    team.bench_points = round(
        sum(
            [
                p.points
                for p in team.roster
                if p.selected_position in bench_positions
            ]
        ),
        2,
    )
    ...
    team.coaching_efficiency, team.optimal_points = metrics.get(
        "coaching_efficiency"
    ).execute_coaching_efficiency(
        team.name,
        team.roster,
        team.points,
        team.positions_filled_active,
...
def get_player_game_time_statuses(week, league: BaseLeague):
    ...
    if not league.dev_offline:
        ...
        response = requests.get(
            "https://www.footballdb.com/transactions/injuries.html",
            headers=headers,
            params=params,
        )
...
if __name__ == "__main__":
    ...
    local_current_nfl_week = get_current_nfl_week(
        config=local_config, dev_offline=False
    )
