...
class LeagueData(object):
    def __init__(
        ...
        espn_auth_json = None
        espn_auth_file = os.path.join(
            base_dir, config.get("ESPN", "espn_auth_dir"), "private.json"
        )
        if os.path.isfile(espn_auth_file):
            ...
            no_auth_msg = (
                '{0}No "private.json" file found for ESPN. If generating the report for a PUBLIC league\n'
        ...
        self.divisions = {
            ...
            for division in self.league_settings_json.get(
                "scheduleSettings"
            ).get("divisions")
        }
        self.num_divisions = (
            len(self.divisions) if len(self.divisions) > 1 else 0
        )
        # only ESPN (out of the supported Yahoo, Sleeper, Fleaflicker, and ESPN platforms) offers home field advantage
        if self.league_settings_json.get("scoringSettings").get(
            "homeTeamBonus"
        ):
            self.home_field_advantage = self.league.settings_json.get(
                "scoringSettings"
            ).get("homeTeamBonus")
        else:
        ...
        self.roster_positions = {
            ...
            for pos_id, pos_count in self.league_settings_json.get(
                "rosterSettings"
            )
        ...
        for week_for_matchups in range(1, self.num_regular_season_weeks + 1):
            ...
            ] = self.league.box_scores(int(week_for_matchups))
            self.matchups_json_by_week[
                str(week_for_matchups)
            ] = self.league.box_data_json

            if int(week_for_matchups) <= int(self.week_for_report):
                scores = []
                for matchup in self.matchups_by_week[
                    str(week_for_matchups)
                ]:  # type: BoxScore
                    for team in [
                        matchup.home_team,
        ...
        for week_for_rosters in range(1, int(self.week_for_report) + 1):
            ...
            team_rosters_json = {}
            for matchup_json in self.matchups_json_by_week[
                str(week_for_rosters)
            ]:
                team_rosters_json[
                    matchup_json["home"]["teamId"]
                ] = matchup_json["home"]["rosterForCurrentScoringPeriod"][
                    "entries"
                ]
                team_rosters_json[
                    matchup_json["away"]["teamId"]
                ] = matchup_json["away"]["rosterForCurrentScoringPeriod"][
                    "entries"
                ]
            self.rosters_json_by_week[
                str(week_for_rosters)
    ...
    def check_auth(self, msg):
        ...
        if use_credentials.lower() == "y":
            logger.info(
                '"{0}y{1}" -> Continuing...'.format(
                    Fore.GREEN, Style.RESET_ALL
                )
            )
        elif use_credentials.lower() == "n":
            logger.info(
                '"{0}n{1}" -> Aborting...'.format(Fore.RED, Style.RESET_ALL)
            )
            sys.exit(0)
        else:
            incorrect_key_msg = (
                '{0}Please type "{1}y{0}" to CONTINUE or "{2}n{0}" to ABORT and press '
    ...
    def map_data_to_base(self, base_league_class):
        ...
        league.url = (
            "https://fantasy.espn.com/football/league?leagueId={0}".format(
                self.league_id
            )
        )
                ...
                for key, matchup_team in matchup_teams.items():
                    ...
                    if int(base_team.team_id) == int(
                        ...
                        base_team.points = float(
                            matchup.home_score + base_team.home_field_advantage
                        )
                    else:
                    ...
                    if (
                        team_json["record"]["division"].get("streakType")
                        == "WIN"
                    ):
                    ...
                    base_team.current_record = BaseRecord(
                        ...
                        ties=int(
                            team_json["record"]["overall"].get("ties", 0)
                        ),
                        percentage=round(
                            float(
                                team_json["record"]["overall"].get(
                                    "percentage", 0
                                )
                            ),
                            3,
                    ...
                    if team_is_home:
                        if (
                            matchup.home_score + self.home_field_advantage
                        ) > matchup.away_score:
            ...
            for team_id, roster in rosters.items():
                ...
                for player in roster:  # type: BoxPlayer
                    ...
                    base_player.nfl_team_abbr = (
                        player.proTeam if player.proTeam != "WSH" else "WAS"
                    )
                    ...
                    if base_player.display_position == "D/ST":
                        ...
                        base_player.first_name = (
                            ...
                            if player_json["firstName"] != "Washington"
                            else "Football Team"
                        )
                    ...
                    base_player.selected_position_is_flex = (
                        True
                        if "/" in player.slot_position
                        and player.slot_position != "D/ST"
                        else False
                    )
...
class LeagueWrapper(League):
    ...
    def _fetch_teams(self):
        ...
        for team in teams:
            ...
            team.owner = sorted(
                [
                    "%s %s" % (owner["firstName"], owner["lastName"])
    ...
    def box_scores(self, week: int = None) -> List[BoxScore]:
        ...
        box_data = [
            BoxScore(matchup, pro_schedule, positional_rankings, week)
