...
class LeagueData(object):
    def __init__(
        ...
        self.league_url = "https://www.fleaflicker.com/nfl/leagues/" + str(
            self.league_id
        )
        scraped_league_info = self.scrape(
            self.league_url,
            os.path.join(self.data_dir, str(self.season), str(self.league_id)),
            str(self.league_id) + "-league-info.html",
        )
        ...
        scraped_league_scores = self.scrape(
            self.league_url + "/scores",
            os.path.join(self.data_dir, str(self.season), str(self.league_id)),
            str(self.league_id) + "-league-scores.html",
        )

        try:
            self.current_week = (
                int(
                    scraped_league_scores.findAll(
            ...
            self.current_week = get_current_nfl_week_function(
                self.config, self.dev_offline
            )

        scraped_league_rules = self.scrape(
            self.league_url + "/rules",
            os.path.join(self.data_dir, str(self.season), str(self.league_id)),
            str(self.league_id) + "-league-rules.html",
        )

        elements = scraped_league_rules.findAll(["dt", "dd"])
        for elem in elements:
            if elem.text.strip() == "Playoffs":
                self.num_playoff_slots = int(
                    elements[elements.index(elem) + 1].span.text.strip()
                )

                playoff_weeks_elements = elements[
                    elements.index(elem) + 1
                ...
                self.num_playoff_slots = config.getint(
                    "Settings", "num_playoff_slots", fallback=6
                )
                self.num_regular_season_weeks = config.getint(
                    "Settings", "num_regular_season_weeks", fallback=13
                )
        ...
        self.week_for_report = week_validation_function(
            self.config, week_for_report, self.current_week, self.season
        )

        # TODO: how to get league rules for LAST YEAR from Fleaflicker API
        self.league_rules = self.query(
            "https://www.fleaflicker.com/api/FetchLeagueRules?leagueId="
            + str(self.league_id),
            os.path.join(self.data_dir, str(self.season), str(self.league_id)),
        ...
        self.league_standings = self.query(
            "https://www.fleaflicker.com/api/FetchLeagueStandings?leagueId="
            + str(self.league_id)
            + ("&season=" + self.season)
            if self.season
            else "",
        ...
        self.ranked_league_teams = sorted(
            self.ranked_league_teams,
            key=lambda x: x.get("recordOverall").get("rank")
        ...
        for wk in range(1, int(self.num_regular_season_weeks) + 1):
            self.matchups_by_week[str(wk)] = self.query(
                "https://www.fleaflicker.com/api/FetchLeagueScoreboard?leagueId="
                + str(self.league_id)
                + "&scoringPeriod="
                + str(wk)
        ...
        for wk in range(1, int(self.week_for_report) + 1):
            self.rosters_by_week[str(wk)] = {
                str(team.get("id")): self.query(
                    "https://www.fleaflicker.com/api/FetchRoster?leagueId="
                    + str(self.league_id)
                    + "&teamId="
                    + str(team.get("id"))
                    + ("&season=" + str(self.season) if self.season else "")
        ...
        self.league_activity = self.query(
            "https://www.fleaflicker.com/api/FetchLeagueActivity?leagueId="
            + str(self.league_id),
            os.path.join(self.data_dir, str(self.season), str(self.league_id)),
            ...
            if season_start < timestamp < season_end:
                if activity.get("transaction"):
                    if activity.get("transaction").get("type"):
                        transaction_type = activity.get("transaction").get(
                            "type"
                        )
                    else:
                    ...
                    if not self.league_transactions_by_team[
                        str(activity.get("transaction").get("team").get("id"))
                    ]:
                        self.league_transactions_by_team[
                            str(
                                activity.get("transaction")
                                .get("team")
                                .get("id")
                            )
                        ] = {
    ...
    def query(self, url, file_dir, filename):
        ...
        if not self.dev_offline:
            logger.debug(
                "Retrieving Fleaflicker data from endpoint: {0}".format(url)
            )
            ...
            try:
                ...
                logger.error(
                    "REQUEST FAILED WITH STATUS CODE: {0} - {1}".format(
            ...
            try:
                logger.debug(
                    "Loading saved Fleaflicker data for endpoint: {0}".format(
                        url
                    )
                )
        ...
        if self.save_data:
            logger.debug(
                "Saving Fleaflicker data retrieved from endpoint: {0}".format(
                    url
                )
            )
            ...
            with open(file_path, "w", encoding="utf-8") as data_out:
                json.dump(
                    response_json, data_out, ensure_ascii=False, indent=2
                )
    ...
    def scrape(self, url, file_dir, filename):
        ...
        if not self.dev_offline:
            logger.debug(
                "Scraping Fleaflicker data from endpoint: {0}".format(url)
            )
        ...
        if self.save_data:
            logger.debug(
                "Saving Fleaflicker data scraped from endpoint: {0}".format(
                    url
                )
            )
            if not os.path.exists(file_dir):
    ...
    def map_data_to_base(self, base_league_class):
        ...
        league.faab_budget = int(
            self.league_info.get("defaultWaiverBudget", 0)
        )
        ...
        for position in self.roster_positions:
            ...
            if position.get("start"):
                ...
                pos_count = (
                    int(position.get("max")) if position.get("max") else 0
                )
            else:
                pos_count = 0
        ...
        for week, matchups in self.matchups_by_week.items():
            ...
            for matchup in matchups:
                ...
                for key in ["home", "away"]:
                    ...
                    opponent_division = self.league_teams[
                    ...
                    league.has_waiver_priorities = (
                        base_team.waiver_priority > 0
                    )
                    base_team.faab = team_data.get(
                    ...
                    base_team.current_record = BaseRecord(
                        ...
                        losses=int(
                            team_data.get("recordOverall", {}).get("losses", 0)
                        ),
                        ties=int(
                        ...
                        division_losses=int(
                            team_data.get("recordDivision", {}).get(
                                "losses", 0
                            )
                        ),
                        division_ties=int(
                    ...
                    base_team.streak_str = (
                        base_team.current_record.get_streak_str()
                    )
                ...
                for player in [
                    ...
                    if flea_league_player:
                        ...
                        base_player.nfl_team_name = (
                            flea_pro_player.get("proTeam", {}).get("location")
                            + " "
                        ...
                        if flea_player_position.get("label") == "D/ST":
                            ...
                            base_player.headshot_url = "https://a.espncdn.com/combiner/i?img=/i/teamlogos/nfl/500/{0}.png".format(
                                base_player.nfl_team_abbr
                            )
                        else:
                        ...
                        base_player.position_type = (
                            "O"
                            if flea_pro_player.get("position")
                            in self.offensive_positions
                            else "D"
                        )
                        ...
                        eligible_positions = [
                            pos
                            for positions in flea_league_player.get(
                                "rankFantasy", {}
                            ).get("positions", {})
                            for pos in positions.get("position", {}).get(
                                "eligibility"
                            )
                        ]
                        ...
                        base_player.selected_position_is_flex = (
                            True
                            if "/" in flea_pro_player.get("position")
                            else False
                        )

                        base_player.status = flea_pro_player.get(
                            "injury", {}
                        ).get("typeAbbreviaition")
                        ...
                        for stat in flea_league_player.get(
                            ...
                            base_stat.stat_id = stat.get("category", {}).get(
                                "id"
                            )
                            base_stat.name = stat.get("category", {}).get(
                                "abbreviation"
                            )
                            base_stat.value = stat.get("value", {}).get(
                                "value", 0
                            )
