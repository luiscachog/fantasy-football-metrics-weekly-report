...
class LeagueData(object):
    def __init__(
        ...
        self.yahoo_data = Data(
            self.data_dir, save_data=save_data, dev_offline=dev_offline
        )
        yahoo_auth_dir = os.path.join(
            base_dir, config.get("Yahoo", "yahoo_auth_dir")
        )
        ...
        if self.game_id and self.game_id != "nfl":
            yahoo_fantasy_game = self.yahoo_data.retrieve(
        ...
        self.league_info = self.yahoo_data.retrieve(
        ...
        self.num_regular_season_weeks = (
            int(self.league_info.settings.playoff_start_week) - 1
        )
        self.num_divisions = int(len(self.league_info.settings.divisions))
        self.divisions = {
            str(division["division"].division_id): division["division"].name
            for division in self.league_info.settings.divisions
        }
        ...
        for wk in range(1, int(self.week_for_report) + 1):
            self.rosters_by_week[str(wk)] = {
                str(team.get("team").team_id): self.yahoo_data.retrieve(
                    str(team.get("team").team_id)
                    + "-"
                    + str(team.get("team").name.decode("utf-8")).replace(
                        " ", "_"
                    )
                    + "-roster",
    ...
    def get_player_data(self, player_key, week=None):
        ...
        if week:
            ...
            return self.yahoo_data.retrieve(
                ...
                params=params,
                new_data_dir=os.path.join(
                    self.data_dir, str(self.season), self.league_id, "players"
                ),
                data_type_class=Player,
            )
    ...
    def map_data_to_base(self, base_league_class):
        ...
        league.is_faab = (
            True if int(self.league_info.settings.uses_faab) == 1 else False
        )
        if league.is_faab:
            league.faab_budget = self.config.getint(
                "Settings", "initial_faab_budget", fallback=100
            )
        ...
        for week, matchups in self.matchups_by_week.items():
            ...
            for matchup in matchups:
                ...
                base_matchup.complete = (
                    True if y_matchup.status == "postevent" else False
                )
                base_matchup.tied = (
                    True
                    if (y_matchup.is_tied and int(y_matchup.is_tied) == 1)
                    else False
                )
                ...
                for team in y_matchup.teams:  # type: dict
                    ...
                    opponent_division = (
                        y_matchup.teams[opposite_key].get("team").division_id
                    )
                    if (
                        team_division
                    ...
                    if league.is_faab:
                        base_team.faab = (
                            int(y_team.faab_balance)
                            if y_team.faab_balance
                            else 0
                        )
        ...
        for week, rosters in self.rosters_by_week.items():
            ...
            for team_id, roster in rosters.items():
                ...
                for player in roster:
                    ...
                    base_player.bye_week = (
                        int(y_player_for_week.bye)
                        if y_player_for_week.bye
                        else 0
                    )
                    base_player.display_position = (
                        y_player_for_week.display_position
                    )
                    base_player.nfl_team_id = (
                        y_player_for_week.editorial_team_key
                    )
                    base_player.nfl_team_abbr = (
                        y_player_for_week.editorial_team_abbr
                    )
                    base_player.nfl_team_name = (
                        y_player_for_week.editorial_team_full_name
                    )
                    ...
                    if base_player.display_position != "DEF":
                        base_player.headshot_url = (
                            "https:"
                            + y_player_for_week.headshot_url.split(":")[-1]
                        )
                    else:
                    ...
                    base_player.owner_team_id = (
                        y_player_for_week.ownership.owner_team_key
                    )
                    base_player.owner_team_name = (
                        y_player_for_week.ownership.owner_team_name
                    )
                    base_player.percent_owned = (
                        float(y_player_for_week.percent_owned_value)
                        if y_player_for_week.percent_owned_value
                        else 0
                    )
                    base_player.points = (
                        float(y_player_for_week.player_points_value)
                        if y_player_for_week.player_points_value
                        else 0
                    )
                    base_player.position_type = y_player_for_week.position_type

                    base_player.primary_position = (
                        y_player_for_week.primary_position
                    )
                    if y_player_for_week.selected_position_value == "W/R":
                        ...
                        y_player_for_week.selected_position_value == "Q/W/R/T"
                    ):
                        ...
                        base_player.selected_position = (
                            y_player_for_week.selected_position_value
                        )
                    base_player.selected_position_is_flex = (
                        True
                        if int(y_player_for_week.selected_position.is_flex)
                        == 1
                        else False
                    )
